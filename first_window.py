...
class MainWindow:
    ...
    def draw_character_selection_menu(self):
        ...
        self.useful_card_start_height = text_h + 20 + self.background_y1 + self.background_height - self.card_height - 100
    ...
    def on_click(self, position, window_name):
        if window_name == 'first_window':
            ...
            if self.character_card_start_height <= position[1] <= self.character_card_start_height + self.character_card_height:
                character_number = round((position[0] - self.character_card_start_width) // (self.character_card_width + 100))
                if self.current_player != -1:
                    if self.current_player == 1:
                        ...
                        if self.player2:
                            ...
                            color = 'blue'
                    pygame.draw.rect(screen, pygame.Color(color),
                                     (self.character_card_start_width + character_number * (self.character_card_width + 100) - 2,
                                      self.character_card_start_height - 2, self.character_card_width + 2, self.character_card_height + 2), 2)
                    pygame.display.flip()
                    return 'second_window'
